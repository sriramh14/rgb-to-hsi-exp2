...
class ModelConfig:
    ...
    def from_dict(cls, values: Dict) -> "ModelConfig":
        """Build a config while tolerating metadata from older checkpoints.

        Some earlier checkpoints contain model-specific fields such as
        ``bb_m_min``.  Ignoring fields that are not part of this dataclass does
        not change the instantiated Stage-1 architecture, and allows those
        checkpoints to be inspected/loaded with the matching model weights.
        """
        values = dict(values)
        ...
        unsupported_keys = sorted(set(values) - valid_keys)
        if unsupported_keys:
            warnings.warn(
                "Ignoring unsupported checkpoint ModelConfig keys: "
...
class DiffIRS1RGB2HSI(nn.Module):
    """Stage 1 with externally controllable HSI contribution to the prior."""
    ...
    def set_hsi_scale(self, scale: float) -> None:
        """Forward the training schedule's HSI scale to the teacher encoder."""
        self.E.set_hsi_scale(scale)

    def get_hsi_scale(self) -> float:
        return self.E.get_hsi_scale()

    def forward_rgb_only(
        self,
        rgb: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Reconstruct HSI using a prior formed without an HSI input image."""
        prior = self.E.forward_rgb_only(rgb)
