...
def stage2_loss_example(
    ...
    """
    Example Stage-2 joint loss.

    total =
        reconstruction loss
        + lambda_prior * prior distillation loss
        + lambda_diffusion * epsilon prediction loss

    The reconstruction term backpropagates through MST++, the prior adapter,
    the complete DDIM trajectory, the denoiser, and the RGB condition encoder.
    """
    ...
    if teacher_prior is None:
        prior_loss = reconstruction_loss.new_zeros(())
    else:
    ...
    return total_loss, {
        ...
        "diffusion_noise_loss": (
            diffusion_noise_loss.detach()
        ),
        "prior_gate": output["prior_gate"].detach(),
    }
...
if __name__ == "__main__":
    ...
    print(
        "Stage 2 train HSI:",
        stage2_training_output["pred_hsi"].shape,
    )
    print(
        "Stage 2 predicted prior:",
        stage2_training_output[
            "predicted_prior"
        ].shape,
    )
    print(
    ...
    print(
        "Stage 2 inference HSI:",
        first["pred_hsi"].shape,
    )
    print(
        "Deterministic prior:",
        torch.allclose(
            first["predicted_prior"],
            second["predicted_prior"],
        ),
    )
