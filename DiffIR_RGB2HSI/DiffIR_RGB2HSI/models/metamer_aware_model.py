...
Main changes in this version:
    ...
    7. CPEN can use a contextual spectral prototype prior head to reduce
       metamer ambiguity without object labels or detectors.
...
class ModelConfig:
    ...
    # Metamerism-aware prior head. Instead of directly projecting CPEN features
    # to 31 bands, this predicts a soft mixture over learned smooth spectral
    # prototypes using local/dilated context. It keeps the same output shape:
    # [B, num_bands, ceil(H/4), ceil(W/4)].
    use_contextual_prototype_prior: bool = True
    num_spectral_prototypes: int = 48
    prototype_hidden_dim: int = 96
...
class CenterMaskedDWConv(nn.Module):
    """Depthwise context convolution with the center coefficient masked out.

    This is used only inside the prototype-selection head. It biases prototype
    weights to depend on surrounding spatial context instead of only the center
    location, which is useful for metamer cases such as green leaf vs green paint.
    """

    def __init__(self, channels: int, kernel_size: int = 5, dilation: int = 1):
        super().__init__()
        if kernel_size % 2 == 0:
            raise ValueError("CenterMaskedDWConv requires an odd kernel_size")
        ...
        mask = torch.ones(channels, 1, kernel_size, kernel_size)
...
class ContextualSpectralPrototypePrior(nn.Module):
    """Metamerism-aware spectral prior head.

    It predicts a soft mixture over K learned 31-band spectral prototypes:
        weights:    [B, K, H, W]
        prototypes: [K, num_bands]
        prior:      [B, num_bands, H, W]

    The prototype-selection branch uses center-masked and dilated depthwise
    convolutions so that local surroundings influence the prior. No object labels,
    segmentation masks, or detectors are used.
    """
    ...
    def __init__(
        ...
        self.context = nn.Sequential(
            ...
            GELU(),
            CenterMaskedDWConv(hidden_channels, kernel_size=5, dilation=1),
            GELU(),
            CenterMaskedDWConv(hidden_channels, kernel_size=5, dilation=2),
            GELU(),
            nn.Conv2d(hidden_channels, hidden_channels, 1),
            GELU(),
        )
        self.weight_head = nn.Conv2d(hidden_channels, num_prototypes, 1)

        # Deterministic, smooth initialization rather than arbitrary random curves.
        init_prototypes = self._make_initial_prototypes(num_prototypes, num_bands)
        self.prototype_logits = nn.Parameter(torch.logit(init_prototypes.clamp(1e-4, 1 - 1e-4)))

    @staticmethod
    def _make_initial_prototypes(num_prototypes: int, num_bands: int) -> torch.Tensor:
        bands = torch.linspace(0.0, 1.0, num_bands).view(1, num_bands)
        centers = torch.linspace(-0.25, 1.25, num_prototypes).view(num_prototypes, 1)
        widths = torch.linspace(0.10, 0.35, num_prototypes).view(num_prototypes, 1)
        bumps = torch.exp(-0.5 * ((bands - centers) / widths).pow(2))
        slopes = torch.linspace(-0.25, 0.25, num_prototypes).view(num_prototypes, 1) * (bands - 0.5)
        curves = 0.20 + 0.60 * bumps + slopes
        return curves.clamp(0.02, 0.98)

    def prototypes(self) -> torch.Tensor:
        """Return physically bounded spectral prototypes [K, num_bands]."""
        return torch.sigmoid(self.prototype_logits)

    def forward_with_weights(self, feat: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        context = self.context(feat)
        logits = self.weight_head(context) / self.temperature
        weights = torch.softmax(logits, dim=1)
        prototypes = self.prototypes()
        spectral_prior = torch.einsum("bkhw,kl->blhw", weights, prototypes)
        return spectral_prior, weights, prototypes

    def forward(self, feat: torch.Tensor) -> torch.Tensor:
        spectral_prior, _, _ = self.forward_with_weights(feat)
        return spectral_prior

    def prototype_smoothness_loss(self) -> torch.Tensor:
        """Second-order spectral smoothness for learned prototype curves."""
        p = self.prototypes()
        if p.shape[1] < 3:
            return p.new_zeros(())
        return (p[:, 2:] - 2.0 * p[:, 1:-1] + p[:, :-2]).abs().mean()

    def prototype_diversity_loss(self) -> torch.Tensor:
        """Penalize prototype collapse using off-diagonal cosine similarity."""
        p = F.normalize(self.prototypes(), dim=1)
        sim = p @ p.t()
        eye = torch.eye(sim.shape[0], device=sim.device, dtype=sim.dtype)
        return ((sim * (1.0 - eye)) ** 2).mean()
...
class SpatialSpectralPriorEncoderBase(nn.Module):
    ...
    def __init__(self, in_channels: int, config: ModelConfig):
        ...
        if config.use_contextual_prototype_prior:
            self.spectral_head = ContextualSpectralPrototypePrior(
                in_channels=feat * 4,
                num_bands=config.num_bands,
                num_prototypes=config.num_spectral_prototypes,
                hidden_channels=config.prototype_hidden_dim,
...
def spectral_prototype_regularization(
    model: nn.Module,
    smooth_weight: float = 1e-4,
    diversity_weight: float = 1e-3,
) -> torch.Tensor:
    """Optional regularizer for all contextual prototype prior heads in a model.

    Add this to the training loss only if use_contextual_prototype_prior=True:
        total_loss = total_loss + spectral_prototype_regularization(model)
    """
    losses: List[torch.Tensor] = []
    for module in model.modules():
        if isinstance(module, ContextualSpectralPrototypePrior):
            losses.append(
                smooth_weight * module.prototype_smoothness_loss()
                + diversity_weight * module.prototype_diversity_loss()
            )
    if not losses:
        try:
...
def collect_spectral_prototypes(model: nn.Module) -> List[torch.Tensor]:
    """Return learned prototype banks from all contextual prior heads."""
    banks: List[torch.Tensor] = []
    for module in model.modules():
        if isinstance(module, ContextualSpectralPrototypePrior):
            banks.append(module.prototypes().detach())
    return banks
