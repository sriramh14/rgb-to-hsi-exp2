...
class DiffusionCPEN(SpatialSpectralPriorEncoderBase):
    """
    Stage-2 diffusion CPEN.

    Training input:
        rgb:         [B, 3, H, W]
        noisy_prior: [B, num_bands, ceil(H/4), ceil(W/4)]
        timestep:    [B]

    Output:
        predicted clean Stage-1 teacher prior x0:
        [B, num_bands, ceil(H/4), ceil(W/4)]

    The RGB branch supplies the condition. The CPEN encoder receives:
        [RGB condition, noisy prior x_t, timestep map]
    and directly predicts the clean teacher prior.
    """

    def __init__(self, config: ModelConfig):
        if config.prior_downsample_factor != 4:
            raise ValueError(
                "DiffusionCPEN currently expects prior_downsample_factor=4"
            )

        factor = config.prior_downsample_factor
        rgb_channels = 3 * (factor ** 2)

        # Input to the actual CPEN:
        #   RGB features      : rgb_channels
        #   noisy prior x_t   : num_bands
        #   timestep map      : 1
        super().__init__(
            in_channels=rgb_channels + config.num_bands + 1,
            config=config,
        )

        self.config = config
        self.rgb_channels = rgb_channels
        self.max_timestep = float(max(config.timesteps - 1, 1))

        # Reuse the useful low-resolution RGB contextual projection from
        # the old RGBConditionEncoder. It no longer predicts a prior itself.
        self.rgb_condition_branch = nn.Sequential(
            nn.Conv2d(
                3,
                rgb_channels,
                kernel_size=3,
    ...
    def encode_rgb_condition(self, rgb: torch.Tensor) -> torch.Tensor:
        if rgb.ndim != 4 or rgb.shape[1] != 3:
            raise ValueError(
                "rgb must have shape [B,3,H,W], "
                f"received {tuple(rgb.shape)}"
        ...
        return self.rgb_condition_branch(rgb_pad)
    ...
    def predict_x0(
        ...
        if noisy_prior.shape[1] != self.config.num_bands:
            raise ValueError(
                f"Expected {self.config.num_bands} prior channels, "
                f"received {noisy_prior.shape[1]}"
            )

        if rgb_condition.ndim != 4:
            raise ValueError("rgb_condition must be a four-dimensional tensor")

        if noisy_prior.shape[0] != rgb_condition.shape[0]:
            raise ValueError("Batch sizes of noisy_prior and RGB condition differ")

        if noisy_prior.shape[-2:] != rgb_condition.shape[-2:]:
            raise ValueError(
                "Spatial sizes of noisy_prior and RGB condition must match. "
                f"Prior={tuple(noisy_prior.shape[-2:])}, "
                f"condition={tuple(rgb_condition.shape[-2:])}"
            )

        batch, _, height, width = noisy_prior.shape
        ...
        if timestep.shape != (batch,):
            raise ValueError(
                f"timestep must have shape [{batch}], "
                f"received {tuple(timestep.shape)}"
            )

        t_map = (
            timestep.to(dtype=noisy_prior.dtype)
            .view(batch, 1, 1, 1)
            / self.max_timestep
        )
        t_map = t_map.expand(batch, 1, height, width)

        diffusion_input = torch.cat(
            [rgb_condition, noisy_prior, t_map],
            dim=1,
        )

        # This CPEN is now the diffusion x0 predictor.
        return self.encode(diffusion_input)
    ...
    def forward(
        ...
        rgb_condition = self.encode_rgb_condition(rgb)
...
class SpatialSpectralPriorDiffusion(nn.Module):
    """x0-prediction diffusion whose denoising network is the Stage-2 CPEN."""
    ...
    def reverse_step(
        ...
        predicted_x0 = self.cpen.predict_x0(
            noisy_prior=prior_t,
            timestep=timestep,
            rgb_condition=rgb_condition,
        )
    ...
    def forward_train(
        ...
        """
        Random-timestep x0 training.

        target_prior must be the detached output of the frozen Stage-1
        TeacherPriorEncoder.
        """
        ...
        # Prevent accidental gradient flow into the Stage-1 teacher.
        target_prior = target_prior.detach()

        batch_size = rgb.shape[0]
        device = rgb.device

        rgb_condition = self.cpen.encode_rgb_condition(rgb)

        expected_shape = (
            batch_size,
            self.config.num_bands,
            rgb_condition.shape[-2],
            rgb_condition.shape[-1],
        )
        if tuple(target_prior.shape) != expected_shape:
            raise ValueError(
                f"Expected teacher prior shape {expected_shape}, "
                f"received {tuple(target_prior.shape)}"
            )
        ...
        # One element preserves compatibility with code that already uses
        # prior_sequence[-1].
    ...
    def sample(
        ...
        batch = rgb_condition.shape[0]
        height, width = rgb_condition.shape[-2:]
        channels = self.config.num_bands
...
class DiffIRS2RGB2HSI(nn.Module):
    """
    Stage 2:
        RGB + noisy prior + timestep
            -> diffusion CPEN
            -> predicted Stage-1 teacher prior
            -> frozen Stage-1 generator
            -> HSI reconstruction
    """
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.G = MSTSpectralDiffIRGeneratorRGB2HSI(config)
        self.diffusion = SpatialSpectralPriorDiffusion(
            config=config,
            cpen=DiffusionCPEN(config),
        )

        self.freeze_generator()

    @property
    def cpen(self) -> DiffusionCPEN:
        return self.diffusion.cpen

    # Optional compatibility alias if older code accesses model.condition.
    @property
    def condition(self) -> DiffusionCPEN:
        return self.diffusion.cpen
    ...
    def train(self, mode: bool = True):
        # model.train() would otherwise put G back into training mode.
        super().train(mode)
        self.G.eval()
        return self
    ...
    def forward(
        ...
        if self.training:
            if target_prior is None:
                raise ValueError(
                    "Stage-2 training requires the detached prior from "
                    "the frozen Stage-1 teacher encoder."
                )

            predicted_prior, prior_sequence = (
            ...
            # G is frozen, but this operation must NOT be wrapped in
            # torch.no_grad(): reconstruction loss may still backpropagate
            # through G into predicted_prior and the diffusion CPEN.
            pred_hsi = self.G(rgb, predicted_prior)

            return pred_hsi, prior_sequence

        predicted_prior = self.diffusion.sample(
            rgb=rgb,
