...
single-GPU only, uses a fixed batch size with no gradient accumulation, follows cosine LR annealing, freezes the complete Stage-2 MST++ generator, deeply supervises the spatial prior network, samples training metrics every 30 batches, and runs exact validation once at the end of every epoch.
"""
...
from models.spec_prior_stage2_frozen import DiffIRS1RGB2HSI, DiffIRS2RGB2HSI, ModelConfig
...
# Stage-2 supervision. The complete pretrained generator G is frozen, so
# these losses force the RGB condition encoder and diffusion denoiser to learn
# the Stage-1 oracle prior rather than allowing the transformer to adapt.
LAMBDA_RECONSTRUCTION = 1.0
LAMBDA_PRIOR_FINAL_L1 = 1.0
LAMBDA_PRIOR_SEQUENCE_L1 = 0.25
LAMBDA_CONDITION_PRIOR_L1 = 0.10
LAMBDA_PRIOR_SAM = 0.05
LAMBDA_PRIOR_KD = 0.0
KD_TEMPERATURE = 0.15
...
def weighted_prior_sequence_l1(
    prior_sequence: list[torch.Tensor],
    target_prior: torch.Tensor,
) -> torch.Tensor:
    """Deeply supervise every diffusion x0 prediction.

    The model returns predictions from the noisiest reverse step to the final
    clean-prior step. Later predictions therefore receive larger weights.
    """
    if not prior_sequence:
        raise ValueError("prior_sequence cannot be empty")

    losses = torch.stack(
        [prior_l1_loss(predicted, target_prior) for predicted in prior_sequence]
    )
    weights = torch.linspace(
        0.25,
        1.0,
        steps=len(prior_sequence),
...
def prior_spectral_angle_loss(
    predicted_prior: torch.Tensor,
    target_prior: torch.Tensor,
    eps: float = 1e-8,
) -> torch.Tensor:
    """Mean spectral-angle loss in radians over the spatial prior map."""
    predicted = predicted_prior.float()
    target = target_prior.float()
    dot = (predicted * target).sum(dim=1)
    predicted_norm = torch.linalg.vector_norm(predicted, ord=2, dim=1)
...
def train() -> None:
    ...
    # Stage 1 trains the complete model. Stage 2 optimizes only the RGB
    # condition encoder and diffusion denoiser; the copied MST++ generator is
    # permanently frozen by spec_prior_frozen_model.py.
    trainable_parameters = [
        parameter for parameter in model.parameters() if parameter.requires_grad
    ]
    if not trainable_parameters:
        raise RuntimeError("No trainable model parameters were found")
    ...
    if RESUME_CHECKPOINT is not None:
        ...
        if "optimizer" in resume:
            try:
                ...
                print(
                    "Checkpoint optimizer groups are incompatible with the "
                    "frozen-generator policy; starting a fresh optimizer. "
                    f"Reason: {exc}",
                    flush=True,
                )
    ...
    if STAGE == 2:
        ...
        print("Stage-2 MST++ generator: permanently frozen")
        print("Stage-2 trainable branch: RGB condition encoder + diffusion denoiser")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        running_prior_sequence_l1 = 0.0
        running_condition_prior_l1 = 0.0
        running_prior_sam = 0.0
        ...
        for batch_index, batch in enumerate(train_loader):
            ...
            with autocast_context(amp_enabled):
                if STAGE == 1:
                    ...
                    sequence_prior_l1 = rec_loss.new_zeros(())
                    condition_prior_l1 = rec_loss.new_zeros(())
                    prior_sam = rec_loss.new_zeros(())
                    ...
                    # The oracle prior is generated by the frozen Stage-1
                    # encoder and is never part of the gradient graph.
                    with torch.no_grad():
                    ...
                    if USE_GRADIENT_CHECKPOINTING:
                        ...
                        pred_hsi, prior_sequence, condition_prior = activation_checkpoint(
                            stage2_forward,
                            rgb,
                            target_prior,
                            use_reentrant=False,
                    ...
                    sequence_prior_l1 = weighted_prior_sequence_l1(
                        prior_sequence,
                        target_prior,
                    )
                    condition_prior_l1 = prior_l1_loss(
                        condition_prior,
                        target_prior,
                    )
                    prior_sam = prior_spectral_angle_loss(
                        predicted_prior,
                        target_prior,
                    )
                    if LAMBDA_PRIOR_KD > 0:
                    ...
                    total_loss = (
                        LAMBDA_RECONSTRUCTION * rec_loss
                        + LAMBDA_PRIOR_FINAL_L1 * prior_l1
                        + LAMBDA_PRIOR_SEQUENCE_L1 * sequence_prior_l1
                        + LAMBDA_CONDITION_PRIOR_L1 * condition_prior_l1
                        + LAMBDA_PRIOR_SAM * prior_sam
            ...
            running_prior_sequence_l1 += float(sequence_prior_l1.item()) * batch_size
            running_condition_prior_l1 += float(condition_prior_l1.item()) * batch_size
            running_prior_sam += float(prior_sam.item()) * batch_size
        ...
        train_prior_sequence_l1 = running_prior_sequence_l1 / max(train_count, 1)
        train_condition_prior_l1 = running_condition_prior_l1 / max(train_count, 1)
        train_prior_sam = running_prior_sam / max(train_count, 1)
        ...
        if STAGE == 1:
            ...
            print(
                ...
                f"| Train SAM {train_metrics['sam']:.4f} "
                f"| Train SSIM {train_metrics['ssim']:.6f} "
                f"| Train Final Prior L1 {train_prior_l1:.6f} "
                f"| Train Sequence Prior L1 {train_prior_sequence_l1:.6f} "
                f"| Train Condition Prior L1 {train_condition_prior_l1:.6f} "
                f"| Train Prior SAM {train_prior_sam:.6f} "
