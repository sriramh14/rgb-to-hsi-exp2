...
from models.spec_prior_with_hsi_weight import DiffIRS1RGB2HSI, DiffIRS2RGB2HSI, ModelConfig
...
# Stage-2 diffusion.
N_DENOISE_RES = 10               #original value was 2
DIFFUSION_TIMESTEPS = 50        #original value was 4
...
# Keep the original full-HSI Stage-1 checkpoint separate from all adapted
# checkpoints so the source model can never be overwritten accidentally.
STAGE1_SOURCE_CHECKPOINT = CHECKPOINT_DIR / "diffir_rgb2hsi_stage1_best.pth"
STAGE1_BEST_PATH = CHECKPOINT_DIR / "diffir_rgb2hsi_stage1_hsi_annealed_best.pth"
STAGE1_BEST_LOSS_PATH = CHECKPOINT_DIR / "diffir_rgb2hsi_stage1_hsi_annealed_best_loss.pth"
STAGE1_LATEST_PATH = CHECKPOINT_DIR / "diffir_rgb2hsi_stage1_hsi_annealed_latest.pth"
...
# Stage 2 should use the completed zero-HSI teacher. Change this to BEST_PATH
# only if that checkpoint was saved with hsi_modulation_scale == 0.
TEACHER_CHECKPOINT = STAGE1_LATEST_PATH

STAGE1_RESUME_CHECKPOINT: Optional[Union[str, Path]] = STAGE1_SOURCE_CHECKPOINT
STAGE2_RESUME_CHECKPOINT: Optional[Union[str, Path]] = None
RESUME_CHECKPOINT: Optional[Union[str, Path]] = None  # selected after parsing --stage
...
def save_checkpoint(
    ...
    *,
    ...
    hsi_schedule_start_epoch: Optional[int] = None,
    hsi_anneal_epochs: Optional[int] = None,
    hsi_zero_hold_epochs: Optional[int] = None,
    hsi_annealing_enabled: bool = False,
) -> None:
    payload = {
        "stage": stage,
        "epoch": epoch,
    ...
    if hsi_modulation_scale is not None:
        payload["hsi_modulation_scale"] = float(hsi_modulation_scale)
    if hsi_schedule_start_epoch is not None:
        payload["hsi_schedule_start_epoch"] = int(hsi_schedule_start_epoch)
    if hsi_anneal_epochs is not None:
        payload["hsi_anneal_epochs"] = int(hsi_anneal_epochs)
    if hsi_zero_hold_epochs is not None:
        payload["hsi_zero_hold_epochs"] = int(hsi_zero_hold_epochs)
    payload["hsi_annealing_enabled"] = bool(hsi_annealing_enabled)
...
def load_stage1_teacher(
    ...
    saved_hsi_scale = float(checkpoint.get("hsi_modulation_scale", 1.0))
    teacher.set_hsi_scale(saved_hsi_scale)
    print(
        f"Loaded frozen Stage-1 teacher with HSI scale "
        f"{teacher.get_hsi_scale():.6f}",
        flush=True,
    )
...
def build_evaluation_model(
    ...
    if STAGE == 1:
        ...
        model.set_hsi_scale(saved_hsi_scale)
    
        print(
            f"Loaded Stage-1 HSI modulation "
            f"scale: {model.get_hsi_scale():.6f}"
        )
    model.eval()
    return model, config
...
def train() -> None:
    ...
    stage1_annealing_active = bool(
        STAGE == 1
        and ENABLE_STAGE1_HSI_ANNEALING
        and RESUME_CHECKPOINT is not None
    )
    freeze_stage1_generator = bool(
        stage1_annealing_active
        and FREEZE_STAGE1_GENERATOR_DURING_ANNEALING
    )

    if freeze_stage1_generator:
        model.G.requires_grad_(False)
        model.G.eval()
        print(
            "Stage-1 generator G is frozen; only the prior encoder E will adapt.",
            flush=True,
        )
    ...
    optimizer = torch.optim.AdamW(
        trainable_parameters,
        lr=(RESUME_LR if stage1_annealing_active and RESUME_LR is not None else LR),
    ...
    start_epoch = 1
    training_end_epoch = NUM_EPOCHS
    hsi_schedule_start_epoch = 1
    best_val_mrae = math.inf
    best_val_loss = math.inf
    epochs_without_improvement = 0

    resume = None
    resume_is_annealing_checkpoint = False
    ...
    if RESUME_CHECKPOINT is not None:
        ...
        if all_config_differences:
            print("Checkpoint/current CONFIG differences:", flush=True)
            for key, (checkpoint_value, current_value) in all_config_differences.items():
                print(
        ...
        if STAGE == 1:
            ...
            if stage1_architecture_differences:
                print("Actual Stage-1 architecture differences:", flush=True)
                for key, (checkpoint_value, current_value) in stage1_architecture_differences.items():
                    print(
                        f"  {key}: checkpoint={checkpoint_value}, current={current_value}",
                        flush=True,
                    )
                raise ValueError(
                    "The checkpoint has a genuinely different Stage-1 architecture."
                )

        model.load_state_dict(resume["model"], strict=True)
        start_epoch = int(resume.get("epoch", 0)) + 1
        resume_is_annealing_checkpoint = bool(
            resume.get("hsi_annealing_enabled", False)
        )

        if stage1_annealing_active:
            if resume_is_annealing_checkpoint:
                hsi_schedule_start_epoch = int(
                    resume.get("hsi_schedule_start_epoch", start_epoch)
                )
                saved_hsi_scale = float(
                    resume.get("hsi_modulation_scale", 1.0)
                )
                model.set_hsi_scale(saved_hsi_scale)
            else:
                # A full-HSI checkpoint starts a new annealing curriculum.
                hsi_schedule_start_epoch = start_epoch
                model.set_hsi_scale(1.0)

            training_end_epoch = (
                hsi_schedule_start_epoch
                + HSI_ANNEAL_EPOCHS
                + HSI_ZERO_HOLD_EPOCHS
                - 1
            )
        else:
            training_end_epoch = NUM_EPOCHS

        optimizer_layout_is_compatible = not (
            freeze_stage1_generator and not resume_is_annealing_checkpoint
        )
        should_restore_optimizer = bool(
            RESTORE_OPTIMIZER_ON_RESUME
            and optimizer_layout_is_compatible
            and "optimizer" in resume
        )
        if should_restore_optimizer:
            optimizer.load_state_dict(resume["optimizer"])
        elif "optimizer" in resume:
            print(
                "Not loading the checkpoint optimizer because the current "
                "trainable parameter set differs from the source run.",
                flush=True,
            )

        if "scaler" in resume and should_restore_optimizer:
            scaler.load_state_dict(resume["scaler"])

        if (
            stage1_annealing_active
            and RESET_BEST_TRACKING_FOR_ANNEALING
            and not resume_is_annealing_checkpoint
        ):
            best_val_mrae = math.inf
            best_val_loss = math.inf
            epochs_without_improvement = 0
        ...
        print(
            f"Resumed Stage {STAGE} from epoch {start_epoch}; "
            f"training will end at epoch {training_end_epoch}.",
            flush=True,
        )

    remaining_epochs = max(1, training_end_epoch - start_epoch + 1)
    ...
    can_restore_scheduler = bool(
        resume is not None
        and not RESTART_SCHEDULER_ON_RESUME
        and "scheduler" in resume
        and not (freeze_stage1_generator and not resume_is_annealing_checkpoint)
    )
    if can_restore_scheduler:
        scheduler_state = resume["scheduler"]
        if isinstance(scheduler_state, dict) and "T_max" in scheduler_state:
            lr_scheduler.load_state_dict(scheduler_state)
        else:
            print(
                "Checkpoint scheduler is incompatible; using a fresh cosine schedule.",
                flush=True,
            )
    elif resume is not None:
        print(
            f"Using a fresh cosine schedule for {remaining_epochs} remaining epochs "
    ...
    print(
        ...
        f"(T_max={remaining_epochs}, eta_min={MIN_LR:.1e})"
    )
    ...
    if stage1_annealing_active:
        print(
            f"HSI curriculum: {HSI_ANNEAL_EPOCHS} annealing epochs + "
            f"{HSI_ZERO_HOLD_EPOCHS} zero-HSI hold epochs",
            flush=True,
        )
    if STAGE == 2:
        print(f"Loaded frozen Stage-1 teacher: {TEACHER_CHECKPOINT}")

    for epoch in range(start_epoch, training_end_epoch + 1):
        if stage1_annealing_active:
            ...
            model.set_hsi_scale(hsi_scale)
            print(
                f"Stage-1 HSI feature scale: "
                f"{model.get_hsi_scale():.6f}",
                flush=True,
            )
        epoch_start = time.perf_counter()
        print(
            f"\nEpoch {epoch}/{training_end_epoch} started "
        ...
        if STAGE == 1:
            print(
                f"Epoch {epoch}/{training_end_epoch} "
        ...
        checkpoint_hsi_scale = (
            model.get_hsi_scale() if STAGE == 1 else None
        )
        checkpoint_schedule_start = (
            hsi_schedule_start_epoch if stage1_annealing_active else None
        )
        checkpoint_anneal_epochs = (
            HSI_ANNEAL_EPOCHS if stage1_annealing_active else None
        )
        checkpoint_zero_hold_epochs = (
            HSI_ZERO_HOLD_EPOCHS if stage1_annealing_active else None
        )

        # During the transition, validation still benefits from non-zero HSI.
        # Do not call such a checkpoint the adapted "best" model. Best-model
        # selection begins only after the encoder is strictly RGB-only.
        eligible_for_best_checkpoint = bool(
            not stage1_annealing_active
            or model.get_hsi_scale() <= 1e-12
        )

        if eligible_for_best_checkpoint and val_results["loss"] < best_val_loss:
            ...
            save_checkpoint(
                ...
                best_val_loss=best_val_loss,
                epochs_without_improvement=epochs_without_improvement,
                hsi_modulation_scale=checkpoint_hsi_scale,
                hsi_schedule_start_epoch=checkpoint_schedule_start,
                hsi_anneal_epochs=checkpoint_anneal_epochs,
                hsi_zero_hold_epochs=checkpoint_zero_hold_epochs,
                hsi_annealing_enabled=stage1_annealing_active,
            )

        if eligible_for_best_checkpoint and val_results["mrae"] < best_val_mrae:
        ...
        elif eligible_for_best_checkpoint:
            ...
            # The curriculum must be allowed to reach zero HSI before early
            # stopping or best-checkpoint selection is considered.
            epochs_without_improvement = 0
            print(
                "HSI annealing is still active; best-checkpoint selection "
                "starts when HSI scale reaches 0."
            )
