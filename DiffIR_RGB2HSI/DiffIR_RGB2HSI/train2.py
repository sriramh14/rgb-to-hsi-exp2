...
This version is corrected for the strict MST++-backbone test.

Key correction
--------------
The reconstruction generator in the corrected `models/DiffIRxMST.py` uses an
MST++-style feature skip:

    RGB -> conv_in(3->31) -> MST body -> conv_out(31->31) -> + conv_in feature

Therefore `ModelConfig.dim` must be equal to `ModelConfig.num_bands`, i.e. 31
for RGB-to-31-band HSI. The earlier train.py used DIM=48 and larger DIRFormer
style block counts, which recreates the mismatch.
...
Notes
-----
- For pure MST++ backbone verification, keep USE_PRIOR_CONDITIONING=False.
  In that mode, the zero prior is truly ignored by G.
- Do not resume checkpoints trained with DIM=48. Start fresh.
- This script accepts tuple/list batches like (rgb, hsi) and dictionary batches
  like {"rgb": rgb, "hsi": hsi}.
"""
...
STAGE = 1                       # 1: Stage-1 oracle prior, 2: Stage-2 diffusion
...
# Stage-2 compact-prior supervision.
LAMBDA_PRIOR_L1 = 1.0
LAMBDA_PRIOR_KD = 0.0
KD_TEMPERATURE = 0.15
...
# ==================================================
# STRICT MST++ BACKBONE ARCHITECTURE
# ==================================================
# These are the important corrected values.
# The old values DIM=48, NUM_BLOCKS=(4,6,6,8), refinement=4, RGB skip=True
# should NOT be used for strict MST++ backbone verification.

FORCE_STRICT_MSTPP_BACKBONE = True

NUM_BANDS = 31
DIM = 31                         # must equal NUM_BANDS for MST++ feature skip
NUM_BLOCKS = (1, 1, 1)            # MST(dim=31, stage=2, num_blocks=[1,1,1])
MST_STAGES = 3                    # MST_Plus_Plus(stage=3)
MST_STAGE_DEPTH = 2               # internal MST has two encoder/decoder levels
MST_FFN_MULT = 4                  # original MST++ FeedForward multiplier
PAD_MULTIPLE = 8                  # reference MST++ pads to multiple of 8
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train DiffIR RGB-to-HSI with corrected MST++ backbone")
    parser.add_argument(
        "--stage",
        type=int,
        choices=[1, 2],
...
def _model_config_field_names() -> set[str]:
    return {field.name for field in fields(ModelConfig)}


def _make_model_config_compatible(**kwargs) -> ModelConfig:
    """Create ModelConfig while ignoring kwargs absent in older files.

    This makes the train script safer if your local ModelConfig has slightly
    different compatibility fields. The corrected DiffIRxMST.py supports all the
    fields used below.
    """
    valid_fields = _model_config_field_names()
    filtered = {key: value for key, value in kwargs.items() if key in valid_fields}
    return ModelConfig(**filtered)


def _set_if_present(config: ModelConfig, name: str, value: Any) -> None:
    if hasattr(config, name):
        setattr(config, name, value)


def enforce_strict_mstpp_config(config: ModelConfig) -> ModelConfig:
    """Force a config to match the corrected MST++ backbone test."""
    if not FORCE_STRICT_MSTPP_BACKBONE:
        return config

    _set_if_present(config, "num_bands", NUM_BANDS)
    _set_if_present(config, "dim", DIM)
    _set_if_present(config, "num_blocks", tuple(NUM_BLOCKS))
    _set_if_present(config, "mst_stages", MST_STAGES)
    _set_if_present(config, "mst_stage_depth", MST_STAGE_DEPTH)
    _set_if_present(config, "mst_ffn_mult", MST_FFN_MULT)
    _set_if_present(config, "pad_multiple", PAD_MULTIPLE)
    _set_if_present(config, "use_prior_conditioning", USE_PRIOR_CONDITIONING)
    _set_if_present(config, "num_refinement_blocks", NUM_REFINEMENT_BLOCKS)
    _set_if_present(config, "use_rgb_to_hsi_skip", USE_RGB_TO_HSI_SKIP)
    _set_if_present(config, "bias", BIAS)
    return config


def validate_strict_mstpp_config(config: ModelConfig, source_name: str) -> None:
    """Fail early if an old DIM=48/DIRFormer-style checkpoint is used."""
    if not FORCE_STRICT_MSTPP_BACKBONE:
        return
    ...
    for name, expected_value in expected.items():
        if hasattr(config, name):
            actual_value = getattr(config, name)
            if isinstance(expected_value, tuple):
                actual_value = tuple(actual_value)
            if actual_value != expected_value:
                problems.append(f"{name}: expected {expected_value}, found {actual_value}")

    if problems:
        joined = "\n  - ".join(problems)
        raise ValueError(
            f"{source_name} is not compatible with the strict MST++ backbone config.\n"
            f"  - {joined}\n"
            "Do not resume/use checkpoints trained with DIM=48 or the previous "
            "PixelUnshuffle/RGB-skip generator. Start a fresh Stage-1 run."
        )


def make_model_config() -> ModelConfig:
    config = _make_model_config_compatible(
        num_bands=NUM_BANDS,
        dim=DIM,
    ...
    config = enforce_strict_mstpp_config(config)
    validate_strict_mstpp_config(config, "Current CONFIG")
    return config
...
def load_stage1_teacher(
    ...
    validate_strict_mstpp_config(teacher_config, f"Teacher checkpoint {path}")
...
def build_training_models(
    ...
    if STAGE == 1:
        model = DiffIRS1RGB2HSI(current_config).to(device)
        return model, None, current_config
    ...
    # Stage 2 must use the same generator architecture as the Stage-1 teacher.
    # Only diffusion-specific hyperparameters are allowed to change.
    ...
    stage2_config = enforce_strict_mstpp_config(stage2_config)
    validate_strict_mstpp_config(stage2_config, "Stage-2 CONFIG")
...
def build_evaluation_model(
    ...
    validate_strict_mstpp_config(config, "Evaluation checkpoint")
...
def validate(
    ...
    for batch in val_loader:
        ...
        if STAGE == 1:
            ...
            noise_kwargs = {
                "size": (rgb.shape[0], model.config.prior_dim),
                "device": device,
            }
...
def train() -> None:
    ...
    if RESUME_CHECKPOINT is not None:
        ...
        validate_strict_mstpp_config(resume_config, f"Resume checkpoint {RESUME_CHECKPOINT}")
    ...
    if FORCE_STRICT_MSTPP_BACKBONE:
        print("Strict MST++ backbone mode: ON")
        print("Using dim=31, num_blocks=(1,1,1), mst_stages=3, no refinement, no RGB skip.")
    if not USE_PRIOR_CONDITIONING:
        print("Prior conditioning: OFF. The generator ignores the prior for pure backbone verification.")
    if STAGE == 2:
